...
APP_TITLE = "DenseJack Tool z3 by 幽幽子的饲养员"
...
class DenseJackApp:
    ...
    def _build_ui(self) -> None:
        ...
        gk = ttk.LabelFrame(self.root, text="手动指定K数（0=自动识别）")
        ...
        ttk.Button(self.root, text="开始转换", command=self._on_start, width=35).pack(pady=10)
    ...
    def _run_single(self, bpm_scale: float, custom_bpm: str, fuzzy_range: int, time_shift_opt: str, k_override: int = 0) -> None:
        ...
        if not path:
            return
        succ, msg = process_single(path, bpm_scale, custom_bpm, fuzzy_range, time_shift_opt, k_override)
